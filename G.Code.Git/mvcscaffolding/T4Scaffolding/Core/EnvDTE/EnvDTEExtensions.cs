...
namespace T4Scaffolding.Core.EnvDTE
{
    public static class EnvDTEExtensions
    {
        ...
        /// the supplied type by looking for a [DisplayColumn] attribute
        /// </summary>
        public static string DisplayNameProperty(this CodeProperty typeInfo)
        {
            try
            {
                var attributes = typeInfo.Attributes.OfType<CodeAttribute2>();
                var me = attributes.FirstOrDefault();
                if (me == null) return string.Empty;
                var firstArgValue = me.Value;
                if (string.IsNullOrEmpty(firstArgValue))
                    return string.Empty;
